...
class ExtractYearTransformation(BaseTransformation):
    transformation_id = "extract_year"
    label = "Extract Year"
    ...
    def apply(self, dataframe: pl.DataFrame, config: dict[str, Any]) -> pl.DataFrame:
        ...
        new_column = config.get("new_column") or config.get("output_column") or f"{column}_year"
        ...
        if dtype == pl.Date or isinstance(dtype, pl.Datetime):
            ...
            expr = pl.col(column).cast(pl.String).str.to_date(strict=False).dt.year()
    ...
    def to_python(self, config: Any = None, *args: Any, **kwargs: Any) -> str:
        ...
        if date_format:
            return (
                "df = df.with_columns(\n"
                f"    pl.col({column!r}).cast(pl.String).str.to_date(format={date_format!r}, strict=False).dt.year().alias({new_column!r})\n"
                ")"
            )

        return (
            "df = df.with_columns(\n"
            f"    pl.col({column!r}).cast(pl.String).str.to_date(strict=False).dt.year().alias({new_column!r})\n"
            ")"
        )
